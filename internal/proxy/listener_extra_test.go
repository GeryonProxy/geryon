...
// Test isModificationQuery with additional cases
func TestIsModificationQuery_Extended(t *testing.T) {
	tests := []struct {
		query    string
		expected bool
	}{
		// Additional cases for edge coverage
		{"  INSERT INTO t VALUES (1)  ", true},  // With whitespace
		{"UPDATE", true},  // Just the keyword
		{"DELETE", true},  // Just the keyword
		{"  SELECT * FROM t", false},  // Leading whitespace
		{"SELECT", false},  // Just SELECT
		{"insert into t", true},  // Lowercase
		{"update t", true},  // Lowercase
		{"delete from t", true},  // Lowercase
		{"CREATE INDEX", true},  // CREATE is modification
		{"create table t", true},  // Lowercase CREATE
		{"TRUNCATE", true},  // TRUNCATE
		{"truncate table t", true},  // Lowercase TRUNCATE
		{"ALTER", true},  // ALTER
		{"alter table t", true},  // Lowercase ALTER
		{"DROP", true},  // DROP
		{"drop index", true},  // Lowercase DROP
		// Note: These cases reveal current function limitations
		// The function does simple prefix matching and doesn't handle CTEs or comments
		{"WITH cte AS (SELECT 1) INSERT INTO t SELECT * FROM cte", false},  // CTE - not detected
		{"/* comment */ INSERT INTO t VALUES (1)", false},  // Comment - not detected
		{"-- comment\nINSERT INTO t VALUES (1)", false},  // Line comment - not detected
	}
