...
func TestProxySession_ID_Increments(t *testing.T) {
	...
	codec := postgresql.NewCodec()
	ps1, _ := NewProxySession(context.Background(), c1, p, codec, nil, cfg, nil, nil, nil, nil, nil, nil, nil, tracing.NewTracer(nil, log), log)
	ps2, _ := NewProxySession(context.Background(), c2, p, codec, nil, cfg, nil, nil, nil, nil, nil, nil, nil, tracing.NewTracer(nil, log), log)
